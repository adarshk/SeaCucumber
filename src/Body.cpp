...
Body::Body(const int32_t segs,const int32_t len,int enum_value):mNumSegs(segs),length(len)
{
    
    got_enum_value = enum_value;
    ...
    switch (got_enum_value) {
        case 0:
            mThorn = Thorn::create(20, 2);
            break;
...
void Body::update()
{
    ...
    //gl::color( Color( 0.1f, 0.2f,  0.8f ) );
    
    sampleCurve();
//    drawPs(2.0);
    
    
    if (got_enum_value == THORN_E) {
        mThorn->setThornLength(5);
        mThorn->update();
    }
...
{
//    int len = 6;
    mLineBase.push_back( vec3( length*2, 0,0 ) );
	mLineBase.push_back( vec3( length*1, 0,0 ) );
	mLineBase.push_back( vec3( length*-1, 0,0 ) );
	mLineBase.push_back( vec3( length*-2, 0, 0 ) );
...
void Body::drawThorns()
{
    ...
    for( int i = 0; i< mPs.size()-1; ++i ){
        for( float j = 0; j < numRot ; ++j ){
            ...
            else if(got_enum_value == CUBE_E){
            mCube->draw(i);
            }
            else if(got_enum_value == SPHERE_E){
            mBall->draw(i);
            }
