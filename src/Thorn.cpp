...
Thorn::Thorn(const int32_t segs, const float len, const int32_t scale0,const int32_t scale1,const int32_t rad ,const int32_t prof):Tube(segs,scale0,scale1),tLength(len),radius(rad),profile(prof)
{
    isWireframe = false;
    //    mNumSegs = 8;
    setSpline();
...
void Thorn::setThornLength(const float &len){
    tLength = len;
}
    
float Thorn::getThornLegnth() const{
    return tLength;
}
...
{
    // = tPerlin.noise(1) * 0.1;
    mLineBase.push_back( vec3( 0, tLength*3, 0 ) );
	mLineBase.push_back( vec3( 0, tLength*2, 0 ) );
	mLineBase.push_back( vec3( 0, tLength, 0 ) );
